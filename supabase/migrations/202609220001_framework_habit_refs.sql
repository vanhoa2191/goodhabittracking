...
alter table public.habit_activities
  add column if not exists framework_habit_id text,
  add column if not exists framework_content_version text;

alter table public.habit_activities
  drop constraint if exists habit_activities_framework_habit_id_check;

alter table public.habit_activities
  add constraint habit_activities_framework_habit_id_check
  check (framework_habit_id is null or framework_habit_id ~ '^GD[1-5]-(NT|SK|MQH|HT|TC)-[0-9]{2}$');

create index if not exists habit_activities_framework_habit_idx
  on public.habit_activities (framework_habit_id)
  where framework_habit_id is not null;

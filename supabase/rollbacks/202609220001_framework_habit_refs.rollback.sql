...
drop index if exists public.habit_activities_framework_habit_idx;
alter table public.habit_activities
  drop constraint if exists habit_activities_framework_habit_id_check,
  drop column if exists framework_content_version,
  drop column if exists framework_habit_id;

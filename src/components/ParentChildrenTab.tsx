...
export function ParentChildrenTab({ onAdjustPoints, onOpenChild }: ParentChildrenTabProps) {
  ...
  const regenerateCode = async (child: ChildProfile) => {
    ...
    await regenerateChildCode(child.id);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">{copy.oneTimeCodeBadge}</span>
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {child.ageStage && <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-900 dark:bg-amber-950/60 dark:text-amber-300">{kidCopy.stageLabels[child.ageStage]} ({child.age !== undefined ? `${child.age} ${kidCopy.ageUnit}` : onboarding.stageLabels[child.ageStage]})</span>}
                  {child.nickname && <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300">🛡️ {child.nickname}</span>}
                </div>
                ...
                <div className="mt-2 flex items-center gap-1.5 flex-wrap text-[11px]">
                  ...
                  <span className={`font-bold px-2 py-0.5 rounded-lg text-[10px] uppercase tracking-wider ${child.showRealNameOnLeaderboard ? 'bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300'}`}>{child.showRealNameOnLeaderboard ? `👤 ${t.modeRealName}` : `🛡️ ${t.modeNickname}`}</span>
                  {!child.isPublicOnLeaderboard && <span className="bg-rose-50 text-rose-600 dark:bg-rose-950/50 dark:text-rose-300 font-semibold px-2 py-0.5 rounded-lg text-[10px]">🔒 {copy.hiddenFromRanking}</span>}
                </div>
                ...
                <div className="mt-3.5 p-3 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/50 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-700 dark:text-indigo-300"><Smartphone className="w-3.5 h-3.5" /><span>{copy.childCode}</span></div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">{copy.forChild(child.name)}</span>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button type="button" onClick={() => { if (childCode) copyChildCode(child.id, childCode); }} disabled={codeActionsDisabled} className="py-1.5 px-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-bold transition-all flex items-center gap-1 shadow-xs cursor-pointer active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-indigo-600 disabled:active:scale-100" title={copy.copyCodeTitle} aria-label={copy.copyCodeTitle}>
                    ...
                    <button type="button" onClick={() => void toggleQr(child.id, childCode)} disabled={codeActionsDisabled} className="py-1.5 px-2.5 rounded-xl bg-white dark:bg-zinc-800 hover:bg-slate-50 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-slate-300 text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white disabled:active:scale-100 dark:disabled:hover:bg-zinc-800" title={copy.qrTitle} aria-label={copy.qrTitle}><QrCode className="w-3.5 h-3.5 text-indigo-600" /><span>QR</span></button>
                  ...
                  {qrChildId === child.id && qrDataUrls[child.id] && (
                    <div className="pt-2 flex flex-col sm:flex-row items-center gap-3 border-t border-indigo-100 dark:border-indigo-900/40 animate-fade-in">
                      <Image src={qrDataUrls[child.id]} alt={`QR Code ${child.name}`} width={80} height={80} unoptimized className="w-20 h-20 rounded-xl border border-slate-200 dark:border-zinc-700 p-1 bg-white shadow-xs" />
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center sm:text-left leading-relaxed">{copy.qrHelp(child.name, childCode || '')}</p>
                    </div>
                  )}
                </div>
              </div>
